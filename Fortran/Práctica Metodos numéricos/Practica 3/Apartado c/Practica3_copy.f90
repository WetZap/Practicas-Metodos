    ...
    if ( fil_1==fil ) then
        ...
        do i = 1, fil
            do j=1,col
                do k = 1, n-1
                    if ( i<=k ) then
                        matriz_supl(i,j)=matriz_supl(i,j)
                        matriz_coef_sup(i)=matriz_coef_sup(i)
                    else
                        matriz_supl(i,j)=matriz_supl(i,j)-((matriz_supl(i,j)/matriz_supl(k,k))*matriz_supl(k,j))
                        matriz_coef_sup(i)=matriz_coef_sup(i)-((matriz_supl(i,k)/matriz_supl(k,k))*matriz_coef_sup(k))
                    end if
                end do
            enddo
        end do
